...
M = (X*Theta').-Y;
J = 0.5 * sum(sum(R.*M.^2))+0.5*lambda*sum(sum(Theta.^2)) + 0.5*lambda*sum(sum(X.^2)) ;

% Unvectorised version for computing X_grad
% for i = 1:num_movies
%   for j = 1:num_users
...
% Unvectorised version for computing Theta_grad
% for i = 1:num_movies
%   for j = 1:num_users
%     if (R(i,j) == 1)
